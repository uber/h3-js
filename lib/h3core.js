 ...
 *                            reading an array
 * @return {number} Cell count (not to exceed JavaScript maximum array length)
 */
function cellCountToInt(cAddress, offset = 0) {
    const lower = C.getValue(cAddress + SZ_CELLCOUNT * offset, 'i32');
    const upper = C.getValue(cAddress + SZ_CELLCOUNT * offset + SZ_INT, 'i32');
    if (upper !== 0) {
