...
/**
 * Error messages corresponding to the core library error code. See
 ...
 * @param {unknown} Value to associate with the error, if any
 * @throws {Error} Error if err is not E_SUCCESS (0)
 */
function throwIfError(errCode, value) {
    if (errCode !== 0) {
        ...
        const err = new Error(
            `${ERROR_MSGS[errCode] || UNKNOWN_ERROR_MSG} (code: ${errCode}${
...
function validateRes(res) {
    if (typeof res !== 'number' || res < 0 || res > 15 || Math.floor(res) !== res) {
        // Error code for E_RES_DOMAIN
        throwIfError(4, res);
    }
}

/**
 * Custom error for unknown units. This error code is not used in the core H3
 * library, only in the JS bindings.
 * @param {unknown} unit Invalid unit
 * @returns {Error}
 */
function UnknownUnitError(unit) {
    const err = new Error(`Unknown unit: ${unit}`);
    err.code = 100;
    return err;
}
...
function cellCountToInt(cAddress, offset = 0) {
    const lower = C.getValue(cAddress + SZ_INT64 * offset, 'i32');
    const upper = C.getValue(cAddress + SZ_INT64 * offset + SZ_INT, 'i32');
    if (upper !== 0) {
...
export function cellToChildren(h3Index, res) {
    ...
    const [lower, upper] = h3IndexToSplitLong(h3Index);
    const count64 = C._malloc(SZ_INT64);
    try {
...
export function uncompactCells(compactedSet, res) {
    ...
    const uncompactCellSize64 = C._malloc(SZ_INT64);
    try {
        throwIfError(
            H3.uncompactCellsSize(set, count, UNUSED_UPPER_32_BITS, res, uncompactCellSize64)
        );
 ...
 */
export function latLngDistance(latlng1, latlng2, unit) {
