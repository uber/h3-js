...
export function latLngToCell(lat, lng, res) {
    ...
    try {
        throwIfError(H3.cellToLatLng(latlng, res, h3Index));
...
export function cellToParent(h3Index, res) {
    const [lower, upper] = h3IndexToSplitLong(h3Index);
    try {
        throwIfError(H3.cellToParent(lower, upper, res, h3Index));
        return readH3IndexFromPointer(h3Index);
    } finally {
        C._free(h3Index);
    }
...
export function cellToCenterChild(h3Index, res) {
    ...
    try {
        H3.cellToCenterChild(lower, upper, res, centerChild);
...
export function polygonToCells(coordinates, res, isGeoJson) {
    ...
    const count64 = C._malloc(SZ_CELLCOUNT);
    try {
        throwIfError(H3.maxPolygonToCellsSize(geoPolygon, res, count64));
        const count = cellCountToInt(count64);
        const hexagons = C._calloc(count, SZ_H3INDEX);
        try {
...
export function compactCells(h3Set) {
    ...
    try {
        throwIfError(H3.compactCells(set, compactedSet, count));
...
export function uncompactCells(compactedSet, res) {
    ...
    const count64 = C.malloc(SZ_CELLCOUNT);
    try {
        throwIfError(H3.maxUncompactSize(set, count, res, count64));
        const cellCount = cellCountToInt(count64);
        // Allocate memory for uncompacted hexagons
        const uncompactedSet = C._calloc(cellCount, SZ_H3INDEX);
        try {
            throwIfError(H3.uncompactCells(set, count, uncompactedSet, cellCount, res));
            return readArrayOfH3Indexes(uncompactedSet, cellCount);
        } finally {
            C._free(set);
