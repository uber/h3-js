...
function throwIfError(err) {
    if (err !== 0) {
        // TODO: Throw a custom error type with the code attached
...
/**
 * Convert number to split long format
 * @private
 * @param {number} num Integer number to convert to split long format
 * @return {number[]} two element array with 32 lower bits and 32 higher bits 
 */
function intToSplitLong(num) {
    if (num > Math.pow(2, 32)) {
        throw new Error(`Number exceeds int32 max: ${num}`);
    }
    return [num, 0];
}
...
function coordinatesToGeoPolygon(coordinates, isGeoJson) {
    ...
    const numHolesOffset = geofenceOffset + SZ_GEOLOOP;
    ...
    if (numHoles > 0) {
        ...
        for (let i = 0; i < numHoles; i++) {
            polygonArrayToGeofence(coordinates[i + 1], holes + SZ_GEOLOOP * i, isGeoJson);
        }
    }
...
function destroyGeoPolygon(geoPolygon) {
    ...
    if (numHoles > 0) {
        ...
        for (let i = 0; i < numHoles; i++) {
            C._free(C.getValue(holes + SZ_GEOLOOP * i + geofenceArrayOffset, 'i8*'));
        }
        C._free(holes);
    }
    C._free(geoPolygon);
...
/**
 * Read a boolean from a pointer to C memory.
 * @private
 * @param  {number} cAddress  Pointer to allocated C memory
...
function readBooleanFromPointer(cAddress, offset = 0) {
    const val = C.getValue(cAddress + SZ_INT * offset * 2, 'i32');
    return Boolean(val);
}
...
function readDoubleFromPointer(cAddress, offset = 0) {
    return C.getValue(cAddress + SZ_DBL * offset * 2, 'double');
}

/**
 * Read a cell count from C memory, and throw if it is out of range
 * of what JavaScript supports.
 * @private
 * @param  {number} cAddress  Pointer to allocated C memory
 * @param {number} offset     Offset, in number of counts, in case we're
 *                            reading an array
 * @return {number} Cell count (not to exceed JavaScript maximum integer)
 */
function cellCountToInt(cAddress, offset = 0) {
    const lower = C.getValue(cAddress + SZ_INT * offset * 2, 'i32');
    const upper = C.getValue(cAddress + SZ_INT * (offset * 2 + 1), 'i32');
    if (upper !== 0) {
        throw new Error(`Cell count is higher than supported in JavaScript`);
    }
    return lower;
}
 ...
 */
function readCellBoundary(geoBoundary, geoJsonCoords, closedLoop) {
...
export function getIcosahedronFaces(h3Index) {
    const [lower, upper] = h3IndexToSplitLong(h3Index);
    const countMemory = C._malloc(SZ_INT);
    try {
        throwIfError(H3.maxFaceCount(lower, upper, countMemory));
        const count = C.getValue(countMemory, 'i32');
...
export function cellToBoundary(h3Index, formatAsGeoJson) {
    const geoBoundary = C._malloc(SZ_CELLBOUNDARY);
    const [lower, upper] = h3IndexToSplitLong(h3Index);
    try {
        throwIfError(H3.cellToBoundary(lower, upper, geoBoundary));
        return readCellBoundary(geoBoundary, formatAsGeoJson, formatAsGeoJson);
    } finally {
        C._free(geoBoundary);
    }
}
...
export function cellToChildren(h3Index, res) {
    ...
    const [lower, upper] = h3IndexToSplitLong(h3Index);
    const count64 = C._malloc(SZ_CELLCOUNT);
    try {
        throwIfError(H3.cellToChildrenSize(lower, upper, res, count64));
        const count = cellCountToInt(count64);
...
export function gridDisk(h3Index, ringSize) {
    ...
    const count64 = C._malloc(SZ_CELLCOUNT);
    try {
        throwIfError(H3.maxGridDiskSize(ringSize, count64));
        const count = cellCountToInt(count64);
        const hexagons = C._calloc(count, SZ_H3INDEX);
        try {
...
export function polygonToCells(coordinates, res, isGeoJson) {
    ...
    const count64 = C._malloc(SZ_CELLCOUNT);
    try {
        throwIfError(H3.maxPolygonToCellsSize(geoPolygon, res, 0, count64));
        const count = cellCountToInt(count64);
        const hexagons = C._calloc(count, SZ_H3INDEX);
        try {
...
export function cellsToMultiPolygon(h3Indexes, formatAsGeoJson) {
    ...
    try {
        ...
        // Clean up
        H3.destroyLinkedMultiPolygon(originalPolygon);
...
export function compactCells(h3Set) {
    ...
    const [countLower, countUpper] = intToSplitLong(count);
    ...
    try {
        throwIfError(H3.compactCells(set, compactedSet, countLower, countUpper));
...
export function uncompactCells(compactedSet, res) {
    ...
    const uncompactCellSize64 = C._malloc(SZ_CELLCOUNT);
    try {
        throwIfError(H3.uncompactCellsSize(set, countLower, countUpper, res, uncompactCellSize64));
        const uncompactCellSize = cellCountToInt(uncompactCellSize64);
        const [uncompactCellSizeLower, uncompactCellSizeUpper] = intToSplitLong(uncompactCellSize);
        // Allocate memory for uncompacted hexagons
        const uncompactedSet = C._calloc(uncompactCellSize, SZ_H3INDEX);
        try {
            throwIfError(H3.uncompactCells(set, countLower, countUpper, uncompactedSet, uncompactCellSizeLower, uncompactCellSizeUpper, res));
...
export function directedEdgeToBoundary(edgeIndex, formatAsGeoJson) {
    ...
    const [lower, upper] = h3IndexToSplitLong(edgeIndex);
    try {
        throwIfError(H3.directedEdgeToBoundary(lower, upper, geoBoundary));
        return readCellBoundary(geoBoundary, formatAsGeoJson);
    } finally {
        C._free(geoBoundary);
    }
}
...
export function gridDistance(origin, destination) {
    ...
    const count64 = C._malloc(SZ_CELLCOUNT);
    try {
        throwIfError(H3.gridDistance(oLower, oUpper, dLower, dUpper, count64));
        return cellCountToInt(count64);
    } finally {
        C._free(count64);
    }
}
...
export function gridPathCells(origin, destination) {
    ...
    const count64 = C._malloc(SZ_CELLCOUNT);
    try {
        throwIfError(H3.gridPathCellsSize(oLower, oUpper, dLower, dUpper, count64));
        const count = cellCountToInt(count64);
        const hexagons = C._calloc(count, SZ_H3INDEX);
        try {
...
export function localIjToCell(origin, coords) {
    ...
    try {
        throwIfError(H3.localIjToCell(...h3IndexToSplitLong(origin), ij, 0, out));
...
export function getNumCells(res) {
    ...
    try {
        ...
        throwIfError(H3.getNumCells(res, count64));

        const lower = C.getValue(count64, 'i32');
        const upper = C.getValue(count64 + SZ_INT, 'i32');
