...
/**
 * Validate an H3 index, throwing an error if invalid
 * @private
 * @param  {mixed} h3index Value to validate
 * @throws {Error}         Error if invalid
 */
function validateIndex(h3Index) {
    if (!h3IsValid(h3Index)) {
        throw new Error(`Invalid index: ${h3Index}`);
    }
}
